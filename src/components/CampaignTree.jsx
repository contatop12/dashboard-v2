...
function NodeMetrics({ node, compact = false, resultsLabel = null }) {
...
function EntityRow({ node, level, onToggleStatus, hasChildren, expanded, onExpand, resultsLabel }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 sm:justify-end">
        <NodeMetrics node={node} resultsLabel={resultsLabel} />
        <Switch
          size="sm"
...
function AdCard({ ad, onToggleStatus, resultsLabel }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2.5 p-3">
        ...
        <NodeMetrics node={ad} compact resultsLabel={resultsLabel} />
      </div>
    </div>
  )
}

function AdsetBlock({ adset, onToggleStatus, labels, resultsLabel }) {
      ...
      {open && hasAds ? (
        <div className="ml-2 border-l border-white/[0.06] pl-3">
          ...
          <div className="flex gap-3 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {ads.map((ad) => (
              <AdCard key={ad.id} ad={ad} onToggleStatus={onToggleStatus} resultsLabel={resultsLabel} />
            ))}
          </div>
        </div>
      ) : null}
...
function CampaignBlock({ campaign, onToggleStatus, labels, resultsLabel }) {
...
export function CampaignTree({ tree, onToggleStatus, labels = DEFAULT_LABELS, resultsLabel = null }) {
