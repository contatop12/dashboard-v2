    ...
    <div className="flex h-full min-h-0 flex-col gap-3 rounded-lg border border-surface-border bg-surface-card p-4">
      ...
      <div className="relative shrink-0">
        ...
        <input
          ...
          className="w-full bg-surface-input border border-surface-border rounded-md pl-7 pr-3 py-1.5 text-xs text-white placeholder:text-muted-foreground focus:outline-none focus:border-brand/50 transition-colors font-sans"
        />
      </div>

      <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto">
        {filtered.map((kw, i) => (
          <div key={kw.keyword} className="group flex items-center gap-2 py-1">
