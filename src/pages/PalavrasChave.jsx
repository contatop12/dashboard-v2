    ...
    <div className="flex flex-col gap-4 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
        <div className="relative flex-1 w-full sm:max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar palavra-chave..." className="w-full bg-surface-card border border-surface-border rounded-md pl-8 pr-4 py-2 text-xs text-white placeholder:text-muted-foreground focus:outline-none focus:border-brand/50 font-sans" />
        </div>
        <div className="flex items-center gap-2">
          {['all', 'Exata', 'Frase', 'Ampla'].map(m => (
            <button key={m} onClick={() => setMatchFilter(m)} className={cn('text-xs px-2 py-2 rounded-md font-sans transition-all', matchFilter === m ? 'bg-brand text-[#0F0F0F] font-semibold' : 'bg-surface-card border border-surface-border text-muted-foreground hover:text-white')}>
              {m === 'all' ? 'Todos' : m}
            </button>
          ))}
        </div>
            ...
            <thead>
              <tr className="border-b border-surface-border bg-surface-input">
                {[
                  ...
                  <th key={col.key} className={cn('px-4 py-2 text-[10px] uppercase tracking-wider font-sans font-medium text-muted-foreground cursor-pointer hover:text-white transition-colors', col.align === 'right' ? 'text-right' : 'text-left')} onClick={() => handleSort(col.key)}>
              ...
              {filtered.map(kw => (
                <tr key={kw.id} className="border-b border-surface-border/50 last:border-0 hover:bg-surface-hover/40 transition-colors">
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                      <div className={cn('w-2 h-2 rounded-full shrink-0', kw.status === 'active' ? 'bg-green-400' : 'bg-yellow-400')} />
                      <span className="font-sans text-white">{kw.keyword}</span>
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <span className={cn('text-[10px] px-2 py-0.5 rounded border font-mono', MATCH_COLORS[kw.matchType])}>{kw.matchType}</span>
                  </td>
                  <td className="px-4 py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <span className={cn('ml-2 font-mono text-[10px]', kw.qualidade >= 7 ? 'text-green-400' : kw.qualidade >= 5 ? 'text-yellow-400' : 'text-red-400')}>{kw.qualidade}</span>
                    </div>
                  </td>
                  <td className="px-4 py-4 text-right font-mono text-muted-foreground">{formatCurrency(kw.bid)}</td>
                  <td className="px-4 py-4 text-right font-mono text-white">{formatCurrency(kw.cpc)}</td>
                  <td className="px-4 py-4 text-right font-mono text-white">{formatNumber(kw.impressoes)}</td>
                  <td className="px-4 py-4 text-right font-mono text-white">{formatNumber(kw.cliques)}</td>
                  <td className="px-4 py-4 text-right font-mono text-white">{formatPercent(kw.ctr)}</td>
                  <td className="px-4 py-4 text-right font-mono text-white">{kw.conversoes}</td>
                  <td className="px-4 py-4 text-right font-mono text-white">#{kw.posicao.toFixed(1)}</td>
                  <td className="px-4 py-4 text-right">
                    <div className={cn('flex items-center justify-end gap-1 font-mono text-[11px]', kw.delta > 0 ? 'text-green-400' : 'text-red-400')}>
